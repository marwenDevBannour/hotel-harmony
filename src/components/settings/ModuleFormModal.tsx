import { useEffect } from 'react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import * as z from 'zod';
import { Module } from '@/hooks/useModulesCrud';
import { ModuleInput } from '@/services/api';
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
} from '@/components/ui/dialog';
import {
  Form,
  FormControl,
  FormField,
  FormItem,
  FormLabel,
  FormMessage,
} from '@/components/ui/form';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';

const moduleSchema = z.object({
  codeM: z.string().min(1, 'Le code est requis').max(20, 'Le code ne peut pas dépasser 20 caractères'),
  libelle: z.string().min(1, 'Le libellé est requis').max(100, 'Le libellé ne peut pas dépasser 100 caractères'),
  ddeb: z.string().min(1, 'La date de début est requise'),
  dfin: z.string().min(1, 'La date de fin est requise'),
});

type ModuleFormData = z.infer<typeof moduleSchema>;

interface ModuleFormModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  module?: Module | null;
  onSubmit: (data: ModuleInput) => void;
  isLoading?: boolean;
}

export function ModuleFormModal({
  open,
  onOpenChange,
  module,
  onSubmit,
  isLoading,
}: ModuleFormModalProps) {
  const isEditing = !!module;

  const form = useForm<ModuleFormData>({
    resolver: zodResolver(moduleSchema),
    defaultValues: {
      codeM: '',
      libelle: '',
      ddeb: new Date().toISOString().split('T')[0],
      dfin: '2099-12-31',
    },
  });

  useEffect(() => {
    if (module) {
      form.reset({
        codeM: module.codeM,
        libelle: module.libelle,
        ddeb: module.ddeb?.split('T')[0] || new Date().toISOString().split('T')[0],
        dfin: module.dfin?.split('T')[0] || '2099-12-31',
      });
    } else {
      form.reset({
        codeM: '',
        libelle: '',
        ddeb: new Date().toISOString().split('T')[0],
        dfin: '2099-12-31',
      });
    }
  }, [module, form]);

  const handleSubmit = (data: ModuleFormData) => {
    onSubmit({
      codeM: data.codeM,
      libelle: data.libelle,
      ddeb: data.ddeb,
      dfin: data.dfin,
    });
    onOpenChange(false);
  };

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>
            {isEditing ? 'Modifier le module' : 'Nouveau module'}
          </DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            <FormField
              control={form.control}
              name="codeM"
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Code</FormLabel>
                  <FormControl>
                    <Input placeholder="CHAMBRES" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
            <FormField
              control={form.control}
              name="libelle"
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Libellé</FormLabel>
                  <FormControl>
                    <Input placeholder="Gestion des chambres" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
            <div className="grid grid-cols-2 gap-4">
              <FormField
                control={form.control}
                name="ddeb"
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Date début</FormLabel>
                    <FormControl>
                      <Input type="date" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <FormField
                control={form.control}
                name="dfin"
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Date fin</FormLabel>
                    <FormControl>
                      <Input type="date" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
            </div>
            <div className="flex justify-end gap-3 pt-4">
              <Button
                type="button"
                variant="outline"
                onClick={() => onOpenChange(false)}
              >
                Annuler
              </Button>
              <Button type="submit" disabled={isLoading}>
                {isLoading ? 'Enregistrement...' : isEditing ? 'Modifier' : 'Créer'}
              </Button>
            </div>
          </form>
        </Form>
      </DialogContent>
    </Dialog>
  );
}
